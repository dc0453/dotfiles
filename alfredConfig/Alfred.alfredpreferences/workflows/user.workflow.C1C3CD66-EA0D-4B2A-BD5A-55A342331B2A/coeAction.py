...
def key_for_record(record):
    return u'{} {} {} {}'.format(record[BRIEF], record[ORG_PATH], record[CREATE_AT], record[OWNER])


def main(workflow):
    query, mis, cache_seconds = get_args()
    incidents_list = workflow.cached_data('coe_favorate_incidents', lambda: query_all_incidents(10),
                                          max_age=int(cache_seconds))
    incidents_list = wf().filter(query, incidents_list, key_for_record)

    if incidents_list:
        for incident in incidents_list:
            workflow.add_item(incident[BRIEF],
                              u'[{}]{} - {} '.format(incident[CREATE_AT], incident[ORG_PATH], incident[OWNER]),
                              arg=incident[INCIDENT_ID], valid=True)
    else:
...
def query_paged_incidents(page_no, page_size=10):
    ...
    resp = web.post(url, data=json.dumps(json_data), headers=headers, allow_redirects=False)
    login_manager.check_login_status(resp)
    resp.raise_for_status()
    return resp.json()
