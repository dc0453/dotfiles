...
def key_for_record(record):
    return u'{} {} {}'.format(record[devToolsAction.GIT_APP_KEY], record[devToolsAction.SERVICE_ALIAS],
                              record[devToolsAction.REPO])


def main(workflow):
    query, mis, cache_seconds = get_args()
    repo_list = workflow.cached_data('repo_my_list_cache', lambda: devToolsAction.query_my_all_code_repos(10),
    ...
    if repo_list:
        for record in repo_list:
            ...
            if 'MAVEN' == service_type:
                ...
                url = 'https://dev.sankuai.com/services/{}/to-deploy/job-list'.format(appkey)
            wf().add_item(
                appkey,
                u'[{}]{}'.format(service_type, repo),
                url,
                valid=True)
    else:
