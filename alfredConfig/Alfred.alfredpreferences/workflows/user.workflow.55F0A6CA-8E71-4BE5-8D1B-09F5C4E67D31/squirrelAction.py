...
def query_all_cluster_of_group(group_name):
    url = 'https://squirrel.mws.sankuai.com/v1/api/clusterGroup/{}/map/list'.format(group_name)
    cookies_str = login_manager.get_cookies()
    headers = {'Cookie': cookies_str, 'env': 'product'}
    params = {}
    resp = web.get(url, params=params, headers=headers, allow_redirects=False)
    ...
    if default_cluster:
        result.append({'unitName': 'default', 'clusterName': default_cluster['clusterName']})
    return result
...
def query_resource(keywords):
    url = 'https://squirrel.mws.sankuai.com/v1/api/commons/queryResource'
    cookies_str = login_manager.get_cookies()
    headers = {'Cookie': cookies_str, 'env': 'product', 'content-type': 'application/json'}
    params = {'offset': 0, 'limit': 20}
    data = {'applicationLocation': 'beijing',
            'buId': '',
            'limit': 10, 'offset': 0,
    ...
    resp = web.post(url, params=params, data=json_data, headers=headers, allow_redirects=False)
    login_manager.check_login_status(resp)
    resp.raise_for_status()
    data = resp.json()['ret']
    records = data['data']
