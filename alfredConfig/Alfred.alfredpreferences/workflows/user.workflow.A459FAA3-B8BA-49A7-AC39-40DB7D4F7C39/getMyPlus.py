...
def main(workflow):
    ...
    plus_dict = workflow.cached_data('plus_releases', lambda: appkeyAction.query_plus_by_mis(mis),
    ...
    if plus_list:
        for release_name in plus_list:
            workflow.add_item(release_name, arg=plus_dict[release_name], largetext=release_name, valid=True)
    else:
