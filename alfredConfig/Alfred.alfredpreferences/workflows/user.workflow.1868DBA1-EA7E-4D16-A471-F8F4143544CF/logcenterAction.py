...
def query_paged_log(url, env='prod'):
...
def query_logs(select_dep_name, env='prod'):
    logs = query_paged_log(
        '/logcenter/rest/logs/?format=json&offset=0&limit=25'
        '&select_dep_name={}'.format(select_dep_name), env)
    next_url = logs['next']
...
def query_my_logs(env='prod'):
    return query_logs('lc_operators', env)


def query_all_logs(env='prod'):
