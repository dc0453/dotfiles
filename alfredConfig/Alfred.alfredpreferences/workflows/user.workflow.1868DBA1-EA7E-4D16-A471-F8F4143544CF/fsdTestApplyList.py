...
def main(workflow):
    ...
    if test_apply_list:
        for test_apply in test_apply_list:
            ...
            online_time_desc = ''
            if online_time:
                online_time_desc = u'上线时间:{}'.format(from_time_stamp(online_time / 1000))
            if source == 1:
                query = "testApplyDetail/{}".format(apply_id)
            else:
                query = "deliveryDetailMed/{}".format(med_id)
            wf().add_item(
                u'[{}]{}'.format(test_apply[fsdAction.FSD_FILED_APPLY_STATUS], test_apply[fsdAction.FSD_FILED_NAME]),
                online_time_desc,
                query, valid=True)
    else:
