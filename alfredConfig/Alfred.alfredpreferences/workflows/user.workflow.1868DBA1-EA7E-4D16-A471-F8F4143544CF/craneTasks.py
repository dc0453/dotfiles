...
def key_for_record(record):
    return u'{} {} {}'.format(record[craneAction.ITEM_TASK_NAME], record[craneAction.ITEM_TASK_DESCRIPTION],
...
def main(workflow):
    ...
    records = wf().cached_data('crane_appkey_tasks_{}'.format(crane_appkey),
                               lambda: craneAction.query_all_tasks_by_appkey(crane_appkey), max_age=int(cache_seconds))
    records = wf().filter(query, records, key_for_record)
    if records:
        for record in records:
            wf().add_item(record[craneAction.ITEM_TASK_NAME],
                          u'[{}][{}] by {} - {}'.format(
