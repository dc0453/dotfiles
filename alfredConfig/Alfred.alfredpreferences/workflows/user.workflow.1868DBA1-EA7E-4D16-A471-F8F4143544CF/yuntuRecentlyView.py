...
def key_for_record(record):
    return u'{} {}'.format(record[yuntuAction.DASHBOARD_NAME], record[yuntuAction.DASHBOARD_OWNER])


def main(workflow):
    query, mis, cache_seconds = get_args()
    records = yuntuAction.query_all_recently_view_dashboards()
    records = wf().filter(query, records, key_for_record)
    if records:
        for record in records:
            wf().add_item(record[yuntuAction.DASHBOARD_NAME],
                          u'{} - {}'.format(record[yuntuAction.DASHBOARD_OWNER],
