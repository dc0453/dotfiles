...
def query_paged_events(pageNum=1, limit=100, category="all", handler="", org_id=None):
    ...
    end_date = utils.unix_timestamp(datetime.combine(today, time(hour=23, minute=59, second=59)))
    json_payload = {
        "id": [],
        "key": "",
        "level": [], "status": [],
        "handler": handler, "source": [], "tag": [],
        "blocked": "0",
        "org_id": org_id,
        ...
        "page_size": limit,
        "category": "all"}
    resp_json = webUtils.post_json(url, login_manager, json_payload=json_payload, headers=headers)
