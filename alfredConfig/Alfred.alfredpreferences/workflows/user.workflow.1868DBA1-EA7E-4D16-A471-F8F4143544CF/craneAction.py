...
def query_appkey_slice(page_num=1, page_size=10, keyword=''):
    url = 'https://crane.mws.sankuai.com/appKey/getAppKeySlice?pageNum={}&pageSize={}&appKey={}'.format(page_num,
                                                                                                        page_size,
                                                                                                        url_encode(
                                                                                                            keyword))
    ...
    if 'result' in result and 'items' in result['result']:
        ...
        } for item in items if item[ITEM_TOTAL_TASK_COUNT] > 0]
    return []
...
def query_task_slice(page_num=1, page_size=10, appkey=''):
    url = 'https://crane.mws.sankuai.com/task/getTaskSlice?pageNum={}&pageSize={}&appKey={}'.format(page_num,
                                                                                                    page_size,
                                                                                                    appkey)
    cookies_str = login_manager.get_cookies()
    headers = {'Cookie': cookies_str}
