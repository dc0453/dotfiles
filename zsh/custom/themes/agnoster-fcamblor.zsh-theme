...
prompt_hg() {
	...
	if $(hg id >/dev/null 2>&1); then
		if $(hg prompt >/dev/null 2>&1); then
			...
			echo -n $(hg prompt "n  {rev}@{branch}") $st
		else
			st=""
...
prompt_status() {
  local symbols
  symbols=()
  [[ $RETVAL -ne 0 ]] && symbols+="%{%F{red}%}b"
  [[ $UID -eq 0 ]] && symbols+="%{%F{yellow}%}b!"
